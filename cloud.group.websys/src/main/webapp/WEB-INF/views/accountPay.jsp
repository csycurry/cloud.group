...
<div class="container mainWidth">
    ...
    <div class="row">
        <div class="alert alert-warning">
            ...
            <p>
                提现说明：提现金额为1元的整数倍，满<span class="red" style="font-size: 18px">1元</span>即可提现,管理员确认后<span class="red" style="font-size: 18px">48小时内</span>支付玩家
            </p>
            <p>
                    ...
                    <div class="panel panel-default" style="padding: 20px">
                        ...
                        <div class="panel-body">
                            <div class="form-group">
                                <p><span style="width:100px;display:inline-block"> 账户余额：</span><span id="ctl00_ContentPlaceHolder1_lbleftmoney" style="color: #478fca !important; line-height: 21px; font-size: 22px">0.30元</span></p>
                            </div>
                            <div class="form-group">
                                <p><span style="width:100px;display:inline-block">正在提现：</span><span id="ctl00_ContentPlaceHolder1_lbaskingmoney" style="color: #478fca !important; line-height: 21px; font-size: 22px">0元</span></p>
                            </div>
                            <div class="form-group">
                                <p><span style="width:100px;display:inline-block">收款账户：</span><span id="ctl00_ContentPlaceHolder1_lbZhiFuBao" style="color: #dd5a43; line-height: 15px; font-size: 16px">wufeng001</span></p>
                            </div>
                            <div class="form-group">
                                <p><span style="width:100px;display:inline-block">手机号码：</span><input name="ctl00$ContentPlaceHolder1$txtphone" type="text" id="ctl00_ContentPlaceHolder1_txtphone" disabled="disabled" placeholder="请先在“基本信息”栏绑定手机号" style="width:300px;" /></p>
                            </div>
                            <div class="form-group">
                                <p><span style="width:100px;display:inline-block">提现金额：</span><input name="ctl00$ContentPlaceHolder1$askmoney" type="text" id="ctl00_ContentPlaceHolder1_askmoney" placeholder="单位是元" style="width:300px;" /></p>
                            </div>
                            <div id="ctl00_ContentPlaceHolder1_div1" class="form-group">
                                <p>
                                    点击获取短信：<input type="button" data-mode="1" value="语音通道1" class="btn btn-sm btn-info phoneverify" />
                                </p>
                            </div>
                            
                            <div class="form-group">
                                <p class="pick">
                                    <input type="submit" name="ctl00$ContentPlaceHolder1$btnpickCash" value="请验证手机号" onclick="return cashclick();" id="ctl00_ContentPlaceHolder1_btnpickCash" class="btn btn-success" style="width: 300px" />
                                </p>
                            </div>
                        </div>
                    </div>
        ...
        <div class="col-md-12">
            <div class="panel panel-default" style="padding: 20px">
                ...
                <table style="width: 100%" class="table table-bordered exptable datatable text-center" id="applicationtable">
                    <thead>
                        <tr>
    ...
    <script type="text/javascript">
        ...
        function cashclick() {
            $('.pick').on('click', '#' + 'ctl00_ContentPlaceHolder1_btnpickCash', function () {
                var money = $('#ctl00_ContentPlaceHolder1_askmoney');
                var lbmoney = money.parent();
                lbmoney.children('span').remove();
                if (money != null && money.val().length > 0) {
                    var reg = /^[1-9]\d*$/;
                    isok = reg.test(money.val());
                    if (!isok) {
                        $("<span class=\"label label-warning\">亲,您输入的提现金额出现错误,请输入整数~</span>").appendTo(lbmoney);
                        return false;
                    }
                    else if (money.val() < 1) {
                        $("<span class=\"label label-warning\">亲,提现金额不能小于1元额~</span>").appendTo(lbmoney);
                        return false;
                    }
        ...
        function DisableOrOpenAllbutton(isopen, count) {
            if (isopen) {
                $('.phoneverify').each(function (i, item) {
                    $(item).val("语音通道" + (i + 1)).removeClass('disabled').removeAttr('disabled style');
                });
            }
            else {
                $('.phoneverify').each(function (i, item) {
                    $(item).val("剩余" + count + '秒').attr('disabled', true).css('cursor', 'not-allowed');;
                });
            }
        }
            ...
            $('.phoneverify').click(
                function () {
                    if ($.cookie("captcha"))
                    {
                        ...
                        $.ajax({
                            url: "Account_AskCash.aspx/GetVerify",
                            type: "post",
                            dataType: "json",
                            contentType: "application/json; charset=utf-8",
                            data: JSON.stringify({ 'mode': $(this).attr('data-mode'), 'phone': $('#' + 'ctl00_ContentPlaceHolder1_txtphone').val() }),
